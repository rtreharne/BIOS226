...
https://autoviva.ninepointeightone.com/app/join/e4fk1GKJ5eWD7a9aFuD8Inwl7Q5dJJ7m/
...
print("1) Data dimensions (rows, columns):")
print(dim(df))

# 2) Confirm the target column exists
print("2) Does the target column 'Subtype' exist?")
print("Subtype" %in% names(df))

# 3) Check class counts and proportions
class_counts <- table(df$Subtype)
print("3a) Class counts:")
print(class_counts)
print("3b) Class proportions:")
print(round(prop.table(class_counts), 3))

# 4) Confirm gene feature columns are present
gene_cols <- grep("^Gene_", names(df), value = TRUE)
print("4) Number of gene feature columns:")
print(length(gene_cols))

# 5) Confirm gene columns are numeric
all_genes_numeric <- all(sapply(df[, gene_cols], is.numeric))
print("5) Are all gene columns numeric?")
print(all_genes_numeric)

# 6) Check for missing values
print("6) Total number of missing values:")
print(sum(is.na(df)))
```
...
- `all_genes_numeric` should be `TRUE`.
- `sum(is.na(df))` should be `0` for this synthetic dataset. There should be no missing data!
...
[Logistic Regression - extra explainer video](https://www.youtube.com/watch?v=yIYKR4sgzI8)
...
You have now completed the core Topic 5 supervised learning workflow.
...
Good machine learning practice is not only about getting a high metric. It is about building an honest, leakage-safe pipeline and choosing an operating threshold that matches real clinical consequences.

Extension for later:

Stress testing (changing noise, signal strength, and class balance) will be completed as a follow-up exercise.
