...
## 9. Getting Ready for Week 2: Installing and loading R packages

Well done on completing this week's workshop. You should now be back up to speed with your RStudio environment.

There's one last thing that I want to discuss and that is R packages.

R packages extend what R can do and typically
...
**Biostrings** is a bioinformatics package from **Bioconductor**, a specialised collection of R packages for analysing biological data such as DNA, RNA, and protein sequences.
Biostrings provides tools for storing, manipulating, and analysing biological sequences in a way that is efficient and biologically meaningful.

You will use Biostrings in next week’s workshop to work directly with sequence data, so we will install it now to make sure everyone’s system is ready.
...
If this runs without errors, Biostrings is ready to use. If you get errors then don't panic, we can help you fix during the workshop.

You must include `library(Biostrings)` near the top of your script whenever you want to use the package.

Try adding and running the following line to your script. Are you able to reproduce the output?

```{r}
alphabetFrequency(DNAString("ACGTACGT"))
```
...
- Install Biostring and run a simple script that uses the package.
